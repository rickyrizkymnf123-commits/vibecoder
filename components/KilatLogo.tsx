...
export function KilatLogo({ size = 'md', showText = true, className = '' }: KilatLogoProps) {
  ...
  return (
    <div className={`flex items-center gap-2.5 select-none ${className}`}>
      {/* Dark Obsidian Glass Badge (Deep Space Carbon, No Ugly Yellow Box) */}
      <div
        className={`relative ${badgeSize} bg-[#0a0f1d] border border-slate-700/80 hover:border-amber-400/60 flex items-center justify-center shadow-lg shadow-black/60 transition-all duration-300 overflow-hidden shrink-0 group`}
      >
        {/* Subtle internal dark-glow */}
        ...
        >
          ...
          />
          {/* Main Speed Force Lightning Bolt */}
          <path
      ...
      {/* Crisp High-Contrast Typography with Smooth Lightning Strike */}
      {showText && (
        <div className="flex items-center gap-1.5 leading-none">
          <span className={`${textSize} font-black tracking-tight font-lightning-strike`}>
            Kilat Tools
          </span>
          {/* Electric energy spark indicator */}
